...
const FeatureCard: React.FC<FeatureCardProps> = ({
  ...
  return <div onClick={onClick} className="bg-gray-800/30 backdrop-blur-sm rounded-3xl p-4 md:p-6 shadow-sm border border-purple-500/20 cursor-pointer hover:shadow-xl hover:scale-105 hover:-translate-y-2 transition-all duration-500 group animate-fade-in hover:border-purple-400/40" style={{
    animationDelay: `${delay}s`
      ...
      <div className="flex items-start space-x-3 md:space-x-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-base md:text-lg mb-2 transition-colors duration-300 text-slate-800">
            {title}
          </h3>
          <p className="text-xs md:text-sm text-purple-300 leading-relaxed">
            {description}
          </p>
        </div>
      </div>
...
export const FeatureGrid: React.FC<FeatureGridProps> = ({
  ...
  return <div className="px-4 md:px-6 mb-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {features.map((feature, index) => <FeatureCard key={feature.key} title={feature.title} description={feature.description} icon={feature.icon} bgColor={feature.bgColor} onClick={() => onFeatureClick(feature.key)} delay={0.7 + index * 0.1} />)}
      </div>
    </div>;
};