...
export const Header: React.FC<HeaderProps> = ({
  ...
  return <header className="fixed top-0 left-0 right-0 z-50 glass border-b border-border/20">
      <div className="px-4 py-3">
        <div className="flex items-center justify-between">
          {/* Left side - Back button or Logo */}
          <div className="flex items-center space-x-3">
            {showBackButton ? <>
                <button onClick={onBack} className="p-2 rounded-lg glass-hover transition-all duration-200 will-change-transform hover:animate-micro-bounce">
                ...
                <div className="flex items-center space-x-3">
                  <div className="relative">
                    <img src={logo} alt="Cadolt AI" className="w-8 h-8 sm:w-10 sm:h-10" />
                  </div>
          ...
          {!showBackButton && <div className="hidden md:flex items-center space-x-1 glass-enhanced rounded-2xl p-1">
              <button onClick={() => onTabChange('dashboard')} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 will-change-transform ${activeTab === 'dashboard' ? 'futuristic-button text-primary-foreground subtle-glow' : 'text-muted-foreground hover:text-foreground glass-hover'}`}>
                Dashboard
              </button>
            </div>}
          ...
          <div className="flex items-center space-x-2">
            ...
            {/* Ask AI Button */}
            <button onClick={onAskAI} className="futuristic-button px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl text-xs sm:text-sm font-medium text-primary-foreground subtle-glow will-change-transform">
              Ask AI
            </button>
            
            {/* User Profile / Login */}
            <button onClick={onProfileClick} className="p-1 rounded-full glass-hover transition-all duration-200 will-change-transform hover:animate-micro-bounce">
              {user ? <Avatar className="w-8 h-8 subtle-glow">
                  <AvatarImage src={getUserProfileImage() || user?.user_metadata?.avatar_url} alt={getDisplayName()} />
                  <AvatarFallback className="bg-primary text-primary-foreground text-sm">
                    {getInitials()}
                  </AvatarFallback>
                </Avatar> : <div className="futuristic-button flex items-center space-x-2 px-3 py-2 rounded-xl text-primary-foreground">
